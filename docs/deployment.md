...
- `i-customize-it.com` / `www.i-customize-it.com` → `frontend-main`
- `admin.i-customize-it.com` → `frontend-admin`
- `api.i-customize-it.com` → Django (includes `/admin/` for Django admin)
...
Set `COMPOSE_PROJECT_NAME` in `.env` before the first production deploy and **do not change it** afterward (Docker volume names depend on it). Existing servers using `i-customize-it-prod-*` containers should set `COMPOSE_PROJECT_NAME=i-customize-it-prod`.

...
iCustomizeIt is a **multi-container** system orchestrated by Docker Compose, with a **pnpm monorepo** for TypeScript frontends and a standalone Django backend.
...
- Call the backend using shared helpers from `@i-customize-it/shared`
- Are built as **Next.js standalone** outputs in production images

### Shared package (`packages/shared`)

Workspace package `@i-customize-it/shared` provides:
...
- **Shared UI** (`packages/shared/src/ui/`) — **Customize Balanced** theme (charcoal primary, brand red/green accents, Rubik + JetBrains Mono via `next/font`, light/dark). Tailwind v4, shadcn primitives, Basecoat CSS. Import via `@i-customize-it/shared/ui`. See [`ui-system.md`](ui-system.md).
- Hooks (`useApi`)
...
Both frontends import `@i-customize-it/shared/ui/styles/globals.css` in root layout and wrap content with `ThemeProvider`.

#### Shared API client

```text
packages/shared/src/api/
  index.ts              # public barrel (@i-customize-it/shared/api)
  clients/
...
- **Single Axios dependency** lives in `@i-customize-it/shared`; both frontends consume clients from here.
...
- **Prod**: HTTPS-only routing by hostname (`i-customize-it.com`, `admin.i-customize-it.com`, `api.i-customize-it.com` in the sample config).

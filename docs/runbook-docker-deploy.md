...
| ------------- | --------------------- | --------------------- |
| `DOCKER_REPO` | `myorg/i-customize-it` | `myorg/i-customize-it` |
...
| `DEPLOY_PATH` | `/opt/i-customize-it` | `/opt/i-customize-it` |
| `COMPOSE_FILE` | `infra/docker/compose/docker-compose.prod.yml` | same |
| `BACKUP_DIR` | `/var/backups/i-customize-it` | `/var/backups/i-customize-it` |
...
| `NEXT_PUBLIC_BACKEND_MAIN_API_URL` | Yes — **public API URL in the browser** (e.g. `https://api.i-customize-it.com`), not `http://backend:8000` |
...
- `DOCKER_REPO` = `myorg/i-customize-it`
...
| App | Image name |
| --- | ---------- |
| Backend | `myorg/i-customize-it-backend` |
| Main frontend | `myorg/i-customize-it-frontend-main` |
| Admin frontend | `myorg/i-customize-it-frontend-admin` |
| Nginx | `myorg/i-customize-it-nginx` |
...
```text
myorg/i-customize-it-backend:staging-260515-1
myorg/i-customize-it-backend:staging-latest
```
...
**Important:** The **Deploy to VM** job runs on the server you set in `VM_HOST` (over SSH). Your laptop can have `infra/env/prod/.env`, but the deploy will still fail until that file exists **inside `DEPLOY_PATH` on the VM** (e.g. `/home/.../deployment/i-customize-it/infra/env/prod/.env`).

1. Install **Docker** and **Docker Compose v2**.
2. Add the deploy user to the `docker` group.
3. **Clone this repository** to `DEPLOY_PATH` (e.g. `/opt/i-customize-it`).
...
6. Set registry deploy keys in the same `.env` for manual deploy: `DOCKER_REPO`, `BRANCH_SLUG`, `DOCKER_USERNAME`, `DOCKER_TOKEN`, `BACKUP_DIR`, `COMPOSE_PROJECT_NAME` (keep `i-customize-it-prod` if containers already use that prefix), `APP_DOMAIN` / `API_HOST` (mirror GitHub Environment variables).
...
9. Create backup directory (or let the deploy script create it with mode `700`):
   ...
   ```bash
   sudo mkdir -p /var/backups/i-customize-it && sudo chown $USER:$USER /var/backups/i-customize-it
   ```
...
| Backup directory | `/var/backups/i-customize-it/` (or `BACKUP_DIR`) |
...
```bash
cd /opt/i-customize-it   # your DEPLOY_PATH

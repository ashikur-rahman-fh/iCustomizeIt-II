...
`@i-customize-it/shared` API tests live under `packages/shared/src/api/core/`:
...
Frontend home page tests assert integration with `@i-customize-it/shared/ui` imports. See [`ui-system.md`](ui-system.md).
...
**Frontend admin auth tests:** `apps/frontend-admin/admin-auth.test.tsx` — run via `npx pnpm@9.15.0 --filter @i-customize-it/frontend-admin test`.

...
| `NEXT_PUBLIC_BACKEND_MAIN_API_URL` | Browser-accessible base URL for `backendMainApi` in `@i-customize-it/shared`. **Production:** must be `https://` and your public API hostname (e.g. `https://api.i-customize-it.com`, same as `API_HOST`). Never use `http://backend:8000` or internal Docker hostnames — CI and Docker builds reject those. Included in frontend CSP `connect-src` at build time. |
...
| Variable               | Purpose                                                                 |
| ---------------------- | ----------------------------------------------------------------------- |
| `COMPOSE_PROJECT_NAME` | Compose project name (container prefix, e.g. `i-customize-it-prod`). **Do not change** after the first deploy or Postgres/Redis volumes will not match. |

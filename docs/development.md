...
**Generated Next.js / TypeScript files:** `next-env.d.ts` and `tsconfig.tsbuildinfo` in the frontend apps are gitignored. Next.js regenerates `next-env.d.ts` (the import path can differ after `next dev` vs `next build`); `tsc --noEmit` writes `tsconfig.tsbuildinfo` as an incremental cache. `pnpm typecheck` and `pnpm test` run `next typegen` first; `make editor-happy` generates types after install. If the editor reports missing Next types on a fresh clone, run `pnpm typecheck` or `bash infra/scripts/dev/generate-next-types.sh`.
...
```ts
import { adminAuthApi, ensureAdminCsrf } from '@i-customize-it/shared/api';
...
**Frontend tests:** `npx pnpm@9.15.0 --filter @i-customize-it/frontend-admin test`
...
```json
"@i-customize-it/shared": "workspace:*"
```
...
Import components from `@i-customize-it/shared/ui` and styles once per app layout. See [`ui-system.md`](ui-system.md) for theming, shadcn CLI usage, and component rules.

```tsx
import { Button, Alert, Navbar } from '@i-customize-it/shared/ui';
import '@i-customize-it/shared/ui/styles/globals.css';
```

Run UI tests: `npx pnpm@9.15.0 --filter @i-customize-it/shared test`

### Backend API client

All browser-to-backend HTTP goes through `@i-customize-it/shared`. **Do not** install or import Axios in `apps/frontend-main` or `apps/frontend-admin`.
...
```ts
import { backendMainApi, isApiError } from '@i-customize-it/shared/api';
...
- **TypeScript:** open a frontend file — `@i-customize-it/shared/...` and `react` should have no red squiggles
